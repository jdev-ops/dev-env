...
VERIFIER_TYPES = config('REPO_VERIFIER_TYPES', default='(feat|fix|bugfix|config|refactor|build|ci|docs|test)')
...
if not re.search(pattern, str(repo.active_branch)):
    print(f"Active branch name is not valid, please follow the pattern:\n {pattern}")
    sys.exit(1)
